// Copyright (c) 2020-2022 szszss. MIT license: third_party/TpacTool/LICENSE.
// Adapted from TpacTool.IO; DDS header fixes for pitch, arrays and cubemaps.
#nullable disable
using System;
using System.IO;
using TpacTool.Lib;
...
namespace TpacTool.IO
{
	public class DdsExporter : AbstractTextureExporter
	{
		...
		public override void Export(Stream writeStream)
		{
            ...
            bool isCube = hasArray && Texture.SystemFlags != null && Texture.SystemFlags.Contains("is_cubemap");
			...
			if (width <= 0 || height <= 0 || Texture.ArrayCount == 0 || Texture.MipmapCount == 0)
				throw new InvalidDataException("Texture dimensions, array count and mip count must be positive.");
			if (isCube && Texture.ArrayCount % 6 != 0)
				throw new InvalidDataException("A cubemap must contain six faces per cube.");
			var images = Texture.TexturePixels.Data.RawImage;
			if (images == null || images.Length != Texture.ArrayCount || SpecifyArray >= images.Length)
				throw new InvalidDataException("Texture array data does not match its header.");
			for (int a = 0; a < images.Length; a++)
			{
				if (images[a] == null || images[a].Length != Texture.MipmapCount)
					throw new InvalidDataException("Texture mip data does not match its header.");
				int w = width, h = height;
				foreach (var mip in images[a])
				{
					long expected = format.IsBlockCompression()
						? (long)Math.Max(1, (w + 3) / 4) * Math.Max(1, (h + 3) / 4) * format.GetBitsPerPixel() * 2
						: (long)w * h * format.GetBitsPerPixel() / 8;
					if (mip == null || mip.LongLength != expected)
						throw new InvalidDataException("Texture pixel data is incomplete.");
					w = Math.Max(w / 2, 1); h = Math.Max(h / 2, 1);
				}
			}
			int pitch = 0;
			if (format.IsBlockCompression())
			{
				...
				pitch = Math.Max(1, ((width + 3) / 4)) * Math.Max(1, ((height + 3) / 4)) * bytesPerBlock;
			...
			if (hasArray && (!isCube || Texture.ArrayCount > 6)) mode = 2;
            if (mode == 2 && format.GetDxgiFormat() == 0) throw new NotSupportedException("Texture format cannot be represented in DDS.");
			...
			using (var headerWriter = new BinaryWriter(new MemoryStream(standardHeader, true)))
			{
				...
				headerWriter.Write(isCube ? (DDSCAPS2_CUBEMAP | DDSCAPS2_CUBEMAP_ALLFACES) : 0);
			...
			if (mode == 2)
			{
				...
				using (var headerWriter = new BinaryWriter(new MemoryStream(dx10Header, true)))
				{
					...
					headerWriter.Write(isCube ? 0x4 : 0);
					headerWriter.Write(hasArray ? (isCube ? Texture.ArrayCount / 6 : Texture.ArrayCount) : 1);
					switch (format)
					{
						case TextureFormat.DXT2:
						case TextureFormat.DXT4:
			...
			{
				if ((ignoreArray && SpecifyArray < 0 && a > 0) || (SpecifyArray >= 0 && a != SpecifyArray))
					continue;
